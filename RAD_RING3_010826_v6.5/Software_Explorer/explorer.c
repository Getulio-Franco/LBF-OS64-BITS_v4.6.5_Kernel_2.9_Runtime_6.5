...
void Coletar_Slots_Inativos(void) {
    ...
    if (!algum_app_ativo && lblStatus) {
        if (strcmp(lblStatus->Graphic.Control.Caption, "LBF-OS RUNTIME v6.4 [KERNEL 2.8] BROWSER TLS 1.2]") != 0) {
            strcpy(lblStatus->Graphic.Control.Caption, "LBF-OS RUNTIME v6.4 [KERNEL 2.8] BROWSER TLS 1.2]");
        }
    }
...
void Setup_Interface_Nativa(void) {
    ...
    lblStatus = gui_create_label((TWinControl*)frmTaskbar, "lblStatus", "LBF-OS RUNTIME v6.4 [KERNEL 2.8] BROWSER TLS 1.2]");
    if (lblStatus) {
